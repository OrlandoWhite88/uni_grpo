#!/usr/bin/env python
"""
Multi-GPU DDP-compatible GRPO fine-tuning script for GPT-OSS 120B using Unsloth.

Launch with:
  accelerate launch grpo_ddp.py [args]
  OR
  torchrun --nproc_per_node N_GPUS grpo_ddp.py [args]
"""
import argparse
import hashlib
...
DEFAULT_OUTPUT_DIR = "grpo_ddp_outputs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run GRPO training for GPT-OSS 120B (Unsloth) with DDP support."
    )
...
def main():
    ...
    grpo_config = GRPOConfig(
        # DDP Configuration - CRITICAL for multi-GPU
    ...
    print("[training] Starting GRPO with DDP support")
    ...
    print(f"[training] To launch with multiple GPUs, use:")
    print(f"           accelerate launch grpo_ddp.py [args]")
    print(f"           OR")
    print(f"           torchrun --nproc_per_node N_GPUS grpo_ddp.py [args]")
    
    train_result = trainer.train()
    print(f"[training] completed - metrics: {train_result.metrics}")

    adapter_out = os.path.join(args.output_dir, "adapter-grpo")
    try:
